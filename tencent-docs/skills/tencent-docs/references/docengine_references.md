...
> ⚠️ **所有 `doc.*` 工具均通过 Poco 的 `doc-mcp` 独立服务调用**，与 `tencent-docs` 主服务平级。四个服务共用连接器 Token，但 endpoint 独立。
...
1. **首选 `image_id`**：先通过 Poco 调用 `tencent-docs` 服务的 `upload_image`，再把返回的 `image_id` 传给本工具。
...
- `file_id` (string, 可选): 文档唯一标识符，与 `file_url` 二选一
- `file_url` (string, 可选): 腾讯文档的文档链接，与 `file_id` 二选一
- `content` (string, 可选): 图片的纯 base64 内容，与 `image_id` 二选一，仅适合不超过当前 MCP 参数限制的小图
- `image_id` (string, 可选): 通过 `tencent-docs` 服务的 `upload_image` 获取，与 `content` 二选一
- `idx` (integer, 必填): 插入位置的索引，从 0 开始
...
1. **首选 `image_id`**：先通过 Poco 调用 `tencent-docs` 服务的 `upload_image`，再传入返回值。
2. **小图可用 `content`**：读取本地图片并传入纯 base64。
3. 图片过大导致参数超限时停止并说明限制，不通过本地脚本直连 MCP。
...
- `image_id` (string, 条件必填): 新图片的 `image_id`，与 `content` 二选一；通过 Poco 调用 `tencent-docs` 服务的 `upload_image` 获取
- `content` (string, 条件必填): 新图片的纯 base64，与 `image_id` 二选一，仅适合不超过当前 MCP 参数限制的小图
- `version_info` (object, 可选): 版本参数，详见《通用说明 > 版本参数》
...
通过 Poco 直接调用 `doc.create_with_markdown`。把 Markdown 内容编码为纯 base64 后传入 `base64_markdown`，不要读取 Poco 凭据、直连 MCP endpoint，也不要把 base64 写入对话或长期文件。

当内容超过当前 MCP 参数限制时，停止并说明限制；不要生成绕过 Poco 的本地调用脚本。
...
### 参数说明
- `base64_markdown` (string, 必填): base64 编码的 Markdown 内容，仅适合不超过当前 MCP 参数限制的内容
