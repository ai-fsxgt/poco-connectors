...
| 调用方式 | 通过 Poco 调用 `slide-mcp` 服务；Token 由连接器注入                                  |
...
| slide_get_style_design_guide | 读 | 获取生成 DESIGN 所需的样式设计指南 |
| slide_get_design | 读 | 读取当前演示文稿已持久化的 DESIGN |
| slide_set_design | 写 | 持久化 DESIGN，作为后续写类操作的前置条件 |
| slide_add_page_with_jsx | 写 | 使用 JSX 添加完整页面；详细流程见 `slide/entry.md` |
| slide_set_text | 写 | 替换指定形状的文本内容 |
...
先调用 `slide_get_info` 获取页数和有序页面列表，再通过 Poco 按需要的 `page_index` 调用 `slide_get_page_info`。只保留任务需要的结构化摘要，不生成直连 MCP 的本地脚本，也不把整份大响应反复放入上下文。
...
1. **首选 `image_id`**：先通过 Poco 调用 `tencent-docs` 服务的 `upload_image`，再把返回的 `image_id` 传给 `slide_add_image`。
2. **小图可用 `content`**：读取本地图片并传入不含 data URI 前缀的纯 base64。不要把 base64 写入对话或长期文件。
3. 图片过大导致 MCP 参数超限时停止并说明限制；不要读取 Poco 凭据、直连 MCP endpoint 或要求用户提供额外开放平台凭据。
...
| `image_id` | string | 否 | 通过 Poco 调用 `tencent-docs` 服务的 `upload_image` 获取，与 `content` 二选一；有效期以服务返回为准 |
| `content` | string | 否 | 图片的纯 base64，与 `image_id` 二选一，仅适合不超过当前 MCP 参数限制的小图 |
...
| `file_id` | string | 否 | 文档ID，与 file_url 二选一 |
| `file_url` | string | 否 | 在线幻灯片的URL链接，与 file_id 二选一 |
| `<addChartArgs>` | object | 否 | 更多参数以 Poco 当前发现的工具 Schema 为准 |
...
| `file_id` | string | 否 | 文档ID，与 file_url 二选一 |
| `file_url` | string | 否 | 在线幻灯片的URL链接，与 file_id 二选一 |
| `<chartTitleArgs>` | object | 否 | 更多参数以 Poco 当前发现的工具 Schema 为准 |
...
| `file_id` | string | 否 | 文档ID，与 file_url 二选一 |
| `file_url` | string | 否 | 在线幻灯片的URL链接，与 file_id 二选一 |
| `<chartLegendArgs>` | object | 否 | 更多参数以 Poco 当前发现的工具 Schema 为准 |
...
| `file_id` | string | 否 | 文档ID，与 file_url 二选一 |
| `file_url` | string | 否 | 在线幻灯片的URL链接，与 file_id 二选一 |
| `<chartAxisArgs>` | object | 否 | 更多参数以 Poco 当前发现的工具 Schema 为准 |
...
| `file_id` | string | 否 | 文档ID，与 file_url 二选一 |
| `file_url` | string | 否 | 在线幻灯片的URL链接，与 file_id 二选一 |
| `<chartSeriesStyleArgs>` | object | 否 | 更多参数以 Poco 当前发现的工具 Schema 为准 |
...
| `file_id` | string | 否 | 文档ID，与 file_url 二选一 |
| `file_url` | string | 否 | 在线幻灯片的URL链接，与 file_id 二选一 |
| `<chartDataLabelsArgs>` | object | 否 | 更多参数以 Poco 当前发现的工具 Schema 为准 |
...
| `file_id` | string | 否 | 文档ID，与 file_url 二选一 |
| `file_url` | string | 否 | 在线幻灯片的URL链接，与 file_id 二选一 |
| `<groupPropertyArgs>` | object | 否 | 更多参数以 Poco 当前发现的工具 Schema 为准 |
...
| `file_id` | string | 否 | 文档ID，与 file_url 二选一 |
| `file_url` | string | 否 | 在线幻灯片的URL链接，与 file_id 二选一 |
| `<commentArgs>` | object | 否 | 更多参数以 Poco 当前发现的工具 Schema 为准 |
...
| `file_id` | string | 否 | 文档ID，与 file_url 二选一 |
| `file_url` | string | 否 | 在线幻灯片的URL链接，与 file_id 二选一 |
| `<commentKeyArgs>` | object | 否 | 更多参数以 Poco 当前发现的工具 Schema 为准 |
...
通过 Poco 直接调用 `slide_set_design`，把完整设计契约放入 `design_md`。不要把凭据写入本地文件，不生成脚本直连 MCP endpoint。

DESIGN 较长时先压缩冗余叙述并保留色板、字体、版式、组件、留白和自检条件；若仍超过当前 MCP 参数限制，停止并说明限制，不绕过 Poco 传输。
...
| `design_md` | string | 是 | DESIGN.md 全文：基于选定风格的版式分类、色板、字体、装饰和自检条件等设计契约 |
...
- 本文档由上游 `slideengine_mcp_tools.go` 派生。如与 Poco 当前发现的工具 Schema 冲突，以当前 Schema 为准。
