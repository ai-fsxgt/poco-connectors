...
- 在线腾讯文档 PPT 全部通过 Poco 调用 `slide-mcp` 服务的 `slide_*` 工具。
...
- 修改某一页时，先整理完整修改清单，优先使用批量工具，避免反复多轮调用。
...
任何 Slide 工作流的第一步都通过 Poco 调用 `slide-mcp`：

1. 调用 `slide_get_info` 获取页数、有序页面 ID 和实际画布尺寸。
2. 按任务需要调用 `slide_get_page_info` 读取相关页面；空白状态判断至少检查现有页面是否只有空占位符。
3. 调用 `slide_get_design` 判断是否已有 DESIGN。

根据实际返回值处理：

| 状态 | 下一步 |
| --- | --- |
| 无查看权限 | 提示用户调整共享权限，不继续写入 |
| 页数或画布尺寸为 0 | 按 0-1 构建整份 PPT |
| 页面只有空占位符 | 按 0-1 构建；新页全部成功后再删除空白页 |
| 没有 DESIGN | 先写 DESIGN，再继续目标任务 |
| 已有 DESIGN | 进入创建、增加页或修改页流程 |

只保留任务需要的结构化摘要，不把 MCP 原始大 JSON 写入长期文件或最终回复。
...
当 `slide_get_design` 表明当前文档没有 DESIGN，或任何写类工具需要 DESIGN 时，先完成本节。后续流程默认已经满足这个前置条件，不再重复声明。

执行步骤：

1. 调用 `slide_get_style_design_guide`，并读取 `sidebar-pptx-generator/references/design-principle.md`。
2. 结合用户需求、当前 PPT 状态、已有内容、材料摘要和搜索事实，在上下文中构思 DESIGN 内容（视觉与落地契约，不含大段叙事）。
3. **禁止写本地 .md 文件**。直接调用 `slide_set_design`，通过 `design_md` 传入完整内容；内容超过当前工具参数限制时停止并说明，不用本地脚本直连 MCP。
...
6. **严格串行生成 N 页 JSX 并调用 `slide_add_page_with_jsx` 写入（严禁并发）**：
   ...
   - 任一页插入失败都停止后续写入并保留原 `page_index`；先核对文档状态，不自动重试或改到其他位置。
...
3. 优先用批量工具按修改清单完成操作；无法批量时按版本顺序逐项调用。
4. 写入后读取目标页一次确认。
...
修改某页时，避免“读一次、改一点、再读、再改”的多轮循环。满足以下任一条件时，先在上下文中整理批量调用方案：
...
优先使用 `slide_add_shapes`、`slide_add_texts`、`slide_add_line_shapes` 等批量工具。没有对应批量工具时按当前版本串行调用；任一写入超时或结果不确定时停止，先只读核对，不生成直连 MCP 的脚本。
