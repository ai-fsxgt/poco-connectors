...
```bash
# 按会议 ID 查询
record_list --meeting-id "100000000"

# 按会议码查询
record_list --meeting-code "123456789"
...
```bash
# 获取录制文件下载地址
record_address --meeting-record-id "record_abc123"
...
```bash
# 获取录制文件的智能纪要（默认原文）
record_smart_minutes --record-file-id "file_abc123"

# 获取中文翻译版纪要
record_smart_minutes \
...
```bash
# 获取转写详情
record_transcript_get --record-file-id "file_abc123"

# 指定起始段落 ID 与查询段落数
record_transcript_get \
  --record-file-id "file_abc123" \
  --meeting-id "100000000" \
  --pid "<paragraph_id>" \
  --limit "30"
...
```bash
# 获取转写段落列表
record_transcript_paragraphs --record-file-id "file_abc123"

# 指定会议 ID
record_transcript_paragraphs \
  --record-file-id "file_abc123" \
  --meeting-id "100000000"
```
...
```bash
# 预览录制权限申请信息
record_permission_apply_prepare --meeting-record-id "record_abc123"

# 同时指定会议 ID
record_permission_apply_prepare \
  --meeting-record-id "record_abc123" \
  --meeting-id "100000000"
```
...
```bash
# 提交录制权限申请
record_permission_apply_commit --meeting-record-id "record_abc123"

# 同时指定会议 ID
record_permission_apply_commit \
  --meeting-record-id "record_abc123" \
  --meeting-id "100000000"
```
...
1. 查询录制列表，获取 meeting_record_id
   record_list --meeting-id "..."

2. 获取录制文件下载地址，获取 record_file_id
   record_address --meeting-record-id <meeting_record_id>

3. 获取智能纪要 / 转写内容
   record_smart_minutes --record-file-id <record_file_id>
   record_transcript_get --record-file-id <record_file_id>
   record_transcript_search --record-file-id <record_file_id> --text "关键词"
```
...
```
1. 调用 prepare 获取预览信息
   record_permission_apply_prepare --meeting-record-id <meeting_record_id>

2. 将 preview 中的「申请类型 / 会议标题 / 录制所有者 / 备注 / 申请人」完整展示给用户，
   并明确询问是否同意发起权限申请；

3. 收到用户明确确认（"确认"/"是"/"yes" 等肯定指令）后，再调用 commit 提交申请：
   record_permission_apply_commit --meeting-record-id <meeting_record_id>
