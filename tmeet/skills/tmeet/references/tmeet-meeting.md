...
```bash
# 修改会议主题
meeting_update --meeting-id "100000000" --subject "新主题"
...
```bash
# 取消普通会议
meeting_cancel --meeting-id "100000000"
...
```bash
# 通过会议 ID 查询（优先级更高）
meeting_get --meeting-id "100000000"

# 通过会议码查询
meeting_get --meeting-code "123456789"
```
...
# 展示所有子会议
meeting_list --show-all-sub 1

# 分页查询（翻下一页）
meeting_list --page-token "<next_page_token>" --page-size 20
```
...
```bash
# 获取会议受邀者列表
meeting_invitees_list --meeting-id "100000000"
