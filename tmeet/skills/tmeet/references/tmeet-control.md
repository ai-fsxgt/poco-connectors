...
> 🔒 **成员来源约束：`kick` 的目标必须是当前会议中的实际参会人，其 `open_id` / `ms_open_id` 必须从 [`report_participants`](tmeet-report.md#participants--获取参会人列表) 的返回结果中获取，严禁使用 `contact_search` 通讯录查询的结果作为来源。**
