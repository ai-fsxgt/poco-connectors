...
连接器 Provider 会在 `upload_file` 调用内部完成类型、扩展名、Content-Type 和大小校验：

1. 用户提供 `content_type` 且有效时优先使用它，否则按扩展名推断；
2. 扩展名和 Content-Type 都无法识别时拒绝处理；
3. 超过对应类型大小限制时在上传前拒绝，不会创建媒体或上传文件。
