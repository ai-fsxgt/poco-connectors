...
description: Turns product or tech specs into concrete Notion tasks that Poco Agent can implement. Breaks down spec pages into detailed implementation plans with clear tasks, acceptance criteria, and progress tracking to guide development from requirements to completion.
---
...
1. **Find spec**: Use `notion-search` to locate specification page
2. **Fetch spec**: Use `notion-fetch` to read specification content
3. **Extract requirements**: Parse and structure requirements from spec
4. **Create plan**: Use `notion-create-pages` for implementation plan
5. **Find task database**: Use `notion-search` to locate tasks database
6. **Create tasks**: Use `notion-create-pages` for individual tasks in task database
7. **Track progress**: Use `notion-update-page` to log progress and update status
...
```
1. Search for spec:
   - Use notion-search with spec name or topic
...
```
1. Fetch spec page:
   - Use notion-fetch with spec URL/ID from search results
...
1. Search for task database:
   - Use notion-search to find "Tasks" or "Task Management" database
...
2. Fetch database schema:
   - Use notion-fetch with database URL/ID
...
1. Create task in task database using notion-create-pages
2. Use parent: { data_source_id: 'data-source-uuid-from-fetch' }
...
**"Can't find spec"**: Use notion-search with spec name/topic, try broader terms, or ask user for URL
