...
本 Skill 说明 POCO 中轻流连接器的操作方式，包括应用搭建、数据管理、审批流转、数据导出等。可用工具以当前任务实际发现的工具为准。

## 认证说明

用户在 POCO 的轻流连接器授权表单中填写轻流授权码（Token）。POCO 加密保存 Token，并通过 HTTP Header `Authorization: Bearer <token>` 传递；不要要求用户在对话或工具参数中提供 Token。

- Token 获取路径：登录轻流 → 左下角头像 → 个人中心 → 账户信息 → 安全信息 → 授权码
- Token 格式以 `mcp_` 开头
- 如遇认证失败，提示用户在 POCO 中重新授权，检查 Token 是否正确或是否已过期

## POCO 运行限制

- 本连接器使用远程 MCP；工具名白名单来自源连接器文档，具体参数以当前发现的工具 Schema 为准。远程服务未返回的工具不可调用，不要自行构造 API 请求替代。
- POCO 每次调用新建 MCP 会话，不保留上一次调用的会话内工作空间选择。本包不开放 `workspace_select`；操作前通过 `auth_whoami`、`workspace_list`、`workspace_get` 确认身份和目标工作空间。工具 Schema 支持工作空间参数时显式传入；无法确定目标时停止操作并说明原因。
- 远程服务不能直接读取 POCO 任务工作区或用户电脑的本地路径。本包不开放 `file_upload_local`、`builder_file_upload_local`、`record_import_repair_local`；导入导出只使用远程工具明确支持的参数和返回内容，不将服务端文件路径当作 POCO 本地文件。
- 写入、删除、审批、导入导出任务创建和代码块执行须通过 POCO 确认。写入发生超时或远端错误后不要自动重试，应先通过只读工具核对是否已经生效。
...
- 每次操作前核对当前工具返回的工作空间及应用标识，不依赖上一次 MCP 会话的工作空间选择
