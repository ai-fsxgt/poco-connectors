...
sheet.get_range_data(file_id, worksheet_id=sheetId, range={rowFrom:0, rowTo:rowTo, colFrom:0, colTo:colTo}) → 读取全部数据
```
...
**步骤 4**：格式美化（`update_rangeData`）

**格式美化**（字体、颜色、对齐、边框）：
```
sheet.update_rangeData(file_id, worksheet_id=sheetId, rangeData=[
  {opType: "format", rowFrom, rowTo, colFrom, colTo, xf: {font: {...}, alc_h: 2, fill: {...}, dg_bottom: 1, ...}}
])
```
...
**数据去重**：先向用户展示拟删除范围并取得确认，再使用 `sheet.delete_range_data` 的当前 schema 删除对应数据；按自下而上的顺序处理，避免行位置变化。
