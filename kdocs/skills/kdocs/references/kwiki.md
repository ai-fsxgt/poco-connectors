...
**普通文件（docx/pptx/pdf/图片等）**：使用 `download_file` 获取下载信息，按实际返回链接下载。

**智能文档**：使用 `read_file` 获取 Markdown，并明确设置 `enable_upload_medias=false`。该工具仍按目录使用敏感写入入口。

**快捷方式**：先定位源文件，再按其类型处理。遇到权限错误停止操作，不绕过访问限制。
