...
在 POCO 中先用 `search_tools` 查找官方工具，再调用目录返回的 `dispatcher`，传入 `name` 与 `arguments`。数组和嵌套对象保持 JSON 类型。
...
> **参数类型要求**：`include_elements` 必须传数组 `["all"]`，在 POCO 工具调用中直接传入 JSON 数组。
...
> 轮询时再次调用同一个 `read_file_content` 工具，带上第一次返回的 `task_id`，并保持 `include_elements` 为数组。
