...
AI PPT 提供一个通用接口 `aippt.execute`，通过 `task_type` 参数路由到不同的生成流水线：
...
- `business_info` (object, 可选): 计费、审核等通用业务信息，不传时服务端按 task_type × mode 自动推导
...
当前目录还包括主题问卷、研究、大纲和生成等分步工具；按 `aippt.` 搜索即可获取完整参数。POCO 单次调用上限 30 秒，长生成可能超时，不保证完整 SSE 生成流程能在本平台完成。
